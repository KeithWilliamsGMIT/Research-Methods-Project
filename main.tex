...
\begin{abstract}
The traditional release process of software involved a major and infrequent release of a system, which was a source of many conflicts and errors. Continuous delivery introduced an alternative approach where releases are small, frequent and incremental, but what are the benefits and challenges of this approach? This paper collates information from a number of sources in an attempt to answer this question. The benefits of continuous delivery include increased productivity, shorter time-to-market, improved customer satisfaction, better communication, improved release reliability and quality, and automated acceptance and unit tests, while challenges include transforming towards continuous delivery, time pressure, technical debt, lower test coverage and an increased quality assurance effort. If these challenges can be overcome, then continuous delivery can add value to any software project.
\end{abstract}

% Note that keywords are not normally used for peerreview papers.
\begin{IEEEkeywords}
Continuous Delivery, Continuous Integration, Continuous Deployment, Benefits, Challenges, DevOps, Agile
\end{IEEEkeywords}}
...
\IEEEPARstart{T}{he} term continuous delivery is a relatively new software development discipline related to releasing a software system. It is closely associated with both the Agile methodology and DevOps. The traditional approach of releasing a system would be to perform testing and bug fixing towards the end of a long development cycle, meaning the time between two subsequent releases. At this point, the developers would hand the software off to the operation engineers who would be responsible for deploying it to production. Mistakes were not uncommon in this scenario due to the amount of manual work involved. Usually, projects that use continuous delivery follow some Agile methodology, which involves short, iterative development cycles. DevOps is a software development practice which aims to bring together development and operations with the end goal of shortening release cycles. This is aided by continuous delivery which offers an alternative to the traditional release approach. This paper will attempt to determine whether or not the payoff of adopting continuous delivery outweighs the challenges which must be overcome. However, we must begin by defining the concepts of both Continuous Integration (CI) and Continuous Delivery (CD).

\subsection{Continuous Integration}
When discussing continuous delivery it is necessary to also mention continuous integration as it is a precondition of continuous delivery. Fowler defines continuous integration as a "software development practice where members of a team integrate their work frequently" \cite{fowler2006continuous}. In other words, the software is built and tested frequently. The article goes on to outline the practices of continuous integration. A single source repository should be maintained. The build should be automated and self-testing to ensure that the system is still functional and stable after making new changes. Everyone should commit to the mainline everyday, where the mainline is the current state of the system. This is important as long periods between commits can lead to a higher possibility of both conflicts occurring, and conflicts having a higher severity. The mainline should be built on the integration machine, also known as a CI server. The build should be fast to ensure quick feedback loops for developers. A feedback loop refers to the time between the developer making a change and the developer receiving the results of that change, for example, if a change caused the tests to pass or fail. If the build fails the developer who made the change is notified of the failure and they then have the option of either fixing the error(s) which caused the failure or revert the change so that the system returns to a functional and stable state. Tests should be carried out in a clone of the production environment to decrease the risk of any unexpected problems occurring when the system is released. Everyone should easily be able to get the latest executable. Finally, this process should be transparent, meaning everyone should be able to see whats happening.

% needed in second column of first page if using \IEEEpubid
%\IEEEpubidadjcol

\subsection{Continuous Delivery}
Continuous delivery is the discipline of ensuring that the system under development is ready to be released to production at any given time, with the aim of making deployments predictable and on-demand affairs. The area of continuous delivery started to gain attention in 2010, when Humble and Farley published their book titled "Continuous Delivery: Reliable Software Releases through Build, Test, and Deployment Automation" \cite{humble2010continuous}. Continuous delivery extends continuous integration and requires the release process to be automated. While continuous integration is a single stage process, continuous delivery is a multi-stage process \cite{LAUKKANEN201755}.
...
It's also worth mentioning the term continuous deployment, which is an extension of continuous delivery where there are no manual steps from the commit stage to the production stage \cite{LAUKKANEN201755}. These terms are often used interchangeably in industry. The benefits and challenges outlined in this paper also apply to continuous deployment. The relationship between continuous integration, continuous delivery and continuous deployment is outlined in Figure \ref{fig:relationship}.
...
\section{Conclusion}
As seen in this paper, the rapid release approach that continuous delivery offers has many benefits over the more traditional big bang approach of large, infrequent releases. However, it comes with a number of challenges which must be overcome before it can be adopted. These changes can often be difficult and time-consuming, particularly when large teams or even a number of teams are involved. However, by understanding these challenges from the outset makes them easier to predict and overcome. While the area of continuous delivery is still in its infancy, it looks quite promising. Due to the evolving needs of customers, it seems that continuous delivery will be adopted more and more by software projects, both big and small, over the next number of years with the aim of providing a better product or service to their customers.
